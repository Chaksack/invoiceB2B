    ...
    <section class="">
      <div class="py-8 px-4 mx-auto lg:px-6">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div class="invoice-list-column col-span-1 space-y-3 max-h-[700px] overflow-y-auto">
            <div class="flex items-center mb-6 gap-4">
              <h2 class="text-xl tracking-tight font-semibold">Invoices</h2>
              <div class="ml-auto flex items-center gap-2">
                <Button @click="fetchInvoices" variant="outline" class="border-gray-400 hover:bg-gray-200 rounded-md" :disabled="isLoadingInvoices">
                  <RefreshCw class="h-4 w-4" :class="{ 'animate-spin': isLoadingInvoices }"/>
                </Button>
                <Dialog v-model:open="isUploadDialogOpen">
                  <DialogTrigger as-child>
                    <Button @click="isUploadDialogOpen = true" class=" bg-primary rounded-md hover:bg-gray-200 transition-colors">
                      <Upload class="h-4 w-4" /> Upload Invoice
                    </Button>
                  </DialogTrigger>
            ...
            <!-- Search Input -->
            <div class="flex ">
              <div class="relative px-2 w-full max-w-sm">
            ...
            <Card v-for="invoice in filteredInvoices" :key="invoice.id"
                  class="rounded-lg p-4 shadow-sm cursor-pointer hover:shadow-md transition-shadow"
                  :class="{ 'border-2 border-blue-500 bg-blue-50': selectedInvoiceForDetail?.id === invoice.id }"
                  @click="selectInvoiceForDetail(invoice)">
              <div class="flex items-center space-x-3">
                <Avatar class="relative overflow-visible h-10 w-10 flex-shrink-0">
                ...
                <div class="flex-grow">
                  <div class="text-md font-semibold">{{ invoice.customerName || 'N/A' }}</div>
                  <div class="text-sm text-muted-foreground">#{{ invoice.invoiceNumber || invoice.id }}</div>
                </div>
                <div class="text-right flex-shrink-0">
                  <div class="text-lg text-black font-semibold">
                    {{ invoice.currency || 'GHS' }} {{ (invoice.totalAmount || 0).toLocaleString() }}
                  </div>
                  <Badge :class="getInvoiceStatusBadgeClass(invoice.status)" class="py-1 px-3 rounded-full text-xs mt-1">
                    <component :is="getInvoiceStatusIcon(invoice.status)" class="w-3 h-3 mr-1"/>
          ...
          <div class="invoice-detail-column col-span-3 p-6 overflow-y-auto">
            <div v-if="selectedInvoiceForDetail">
              <!-- Top Section: Client Details, Bank Details & Basic Info -->
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                <div class="col-span-2">
                  <div class="left-section-top-details mb-4 space-y-4">
                    <!-- Client Details Card -->
                    <div class="bg-gray-700 text-white p-4 rounded-lg shadow-sm">
                      <h3 class="text-lg font-semibold mb-2">Invoice Details</h3>
                      <div class="flex items-center space-x-3 mb-2">
                        <Avatar class="h-12 w-12 flex-shrink-0">
                          <AvatarImage :src="selectedInvoiceForDetail.customer?.avatarUrl || 'https://placehold.co/48x48/CCCCCC/FFFFFF?text=JD'" class="rounded-full" alt="Client Avatar" />
                          <AvatarFallback class="bg-gray-300 rounded-full flex items-center justify-center">
                            {{ selectedInvoiceForDetail.customerName ? selectedInvoiceForDetail.customerName.substring(0, 2).toUpperCase() : '?' }}
                          </AvatarFallback>
                        </Avatar>
                        <div>
                          <p class="font-semibold text-lg">{{ selectedInvoiceForDetail.customerName || 'N/A' }}</p>
                          <p class="text-sm ">{{ selectedInvoiceForDetail.customer?.email || 'N/A Email' }}</p>
                        </div>
                        <div class="ml-auto">
                          <p class="text-sm  mt-1">{{ selectedInvoiceForDetail.customerAddress?.street || 'Street not available' }}, {{ selectedInvoiceForDetail.customerAddress?.city || 'City not available' }}</p>
                          <p class="text-sm ">{{ selectedInvoiceForDetail.customerAddress?.zipCode }} {{ selectedInvoiceForDetail.customerAddress?.country }}</p>
                        </div>
                      </div>
                    </div>
                    <!-- Right Slot: Basic Info -->
                    <div class="bg-gray-50 p-4 rounded-lg shadow-sm space-y-2 h-full flex flex-col">
                      <h3 class="text-lg font-semibold">Basic Info</h3>
                      <div class="flex justify-between items-center text-sm">
                        <p class="font-semibold">Invoice #:</p>
                        <p>{{ selectedInvoiceForDetail.invoiceNumber || selectedInvoiceForDetail.id }}</p>
                      </div>
                      <div class="flex justify-between items-center text-sm">
                        <p class="font-semibold">Invoice Date:</p>
                        <p>{{ formatDate(selectedInvoiceForDetail.invoiceDate) }}</p>
                      </div>
                      <div class="flex justify-between items-center text-sm">
                        <p class="font-semibold">Due Date:</p>
                        <p>{{ formatDate(selectedInvoiceForDetail.dueDate) }}</p>
                      </div>
                      <div class="flex justify-between items-center text-sm">
                        <p class="font-semibold">Payment Terms:</p>
                        <p>{{ selectedInvoiceForDetail.paymentTerms || 'N/A' }}</p>
                      </div>
                    </div>
                  </div>
                  <!-- Status Tracker -->
                      ...
                      <template v-for="(step, index) in predefinedSteps" :key="step.step">
                        <div class="flex flex-col items-center w-1/5">
                          <div
                              ...
                              :class="{
                          'bg-blue-600 text-white': getStepForInvoiceStatus(selectedInvoiceForDetail.status) === step.step,
                          'bg-green-500 text-white': getStepForInvoiceStatus(selectedInvoiceForDetail.status) > step.step,
                          'bg-gray-300 text-gray-600': getStepForInvoiceStatus(selectedInvoiceForDetail.status) < step.step
                        }"
                          >
                          ...
                          <p
                              ...
                              :class="{
                          'text-blue-600': getStepForInvoiceStatus(selectedInvoiceForDetail.status) === step.step,
                          'text-green-500': getStepForInvoiceStatus(selectedInvoiceForDetail.status) > step.step,
                          'text-gray-500': getStepForInvoiceStatus(selectedInvoiceForDetail.status) < step.step
                        }"
                          >
                            {{ step.title }}
                          </p>
                        ...
                        <div
                            ...
                            :class="{
                        'bg-green-500': getStepForInvoiceStatus(selectedInvoiceForDetail.status) > step.step,
                        'bg-gray-300': getStepForInvoiceStatus(selectedInvoiceForDetail.status) <= step.step
                      }"
                        ></div>
                      </template>
                    </div>
                  ...
                  <Table v-if="selectedInvoiceForDetail.items && selectedInvoiceForDetail.items.length > 0" class="mt-2 w-full text-sm">
                    <TableHeader>
                      <TableRow>
                    ...
                    <TableBody>
                      <TableRow v-for="item in selectedInvoiceForDetail.items" :key="item.id">
                        <TableCell>
                          <div>{{ item.name }}</div>
                          <div class="text-xs font-light text-muted-foreground">{{ item.description }}</div>
                        </TableCell>
                        <TableCell class="text-right">{{ item.quantity }}</TableCell>
                        <TableCell class="text-right">{{ selectedInvoiceForDetail.currency || 'GHS' }} {{ item.unitPrice?.toLocaleString() || '0.00' }}</TableCell>
                        <TableCell class="text-right">{{ selectedInvoiceForDetail.currency || 'GHS' }} {{ item.amount?.toLocaleString() || '0.00' }}</TableCell>
                      </TableRow>
                    </TableBody>
                  </Table>
                  <p v-else class="mt-2 text-muted-foreground text-sm">No items listed for this invoice.</p>

                  <!-- Totals -->
                  <div class="mt-6 bg-gray-100 p-4 rounded-md text-sm">
                    <h3 class="font-semibold mb-2">Invoice Totals</h3>
                    <div class="flex justify-between items-center mb-1">
                      <p>Sub Total:</p>
                      <p class="font-semibold">{{ selectedInvoiceForDetail.currency || 'GHS' }} {{ selectedInvoiceForDetail.subTotalAmount?.toLocaleString() || '0.00' }}</p>
                    </div>
                    <div class="flex justify-between items-center mb-1">
                      <p>Tax ({{ selectedInvoiceForDetail.taxRatePercentage || 0 }}%):</p>
                      <p class="font-semibold">{{ selectedInvoiceForDetail.currency || 'GHS' }} {{ selectedInvoiceForDetail.taxAmount?.toLocaleString() || '0.00' }}</p>
                    </div>
                    <hr class="my-2 border-gray-300">
                    <div class="flex justify-between items-center font-bold text-lg">
                      <p>Total Amount:</p>
                      <p>{{ selectedInvoiceForDetail.currency || 'GHS' }} {{ selectedInvoiceForDetail.totalAmount?.toLocaleString() || '0.00' }}</p>
                    </div>
                    <div class="flex justify-between items-center text-green-600 font-bold text-lg">
                      <p>Balance Due:</p>
                      <p>{{ selectedInvoiceForDetail.currency || 'GHS' }} {{ selectedInvoiceForDetail.balanceDue?.toLocaleString() || '0.00' }}</p>
                    </div>
                  </div>

                  <!-- Action Buttons (moved and re-styled for the detail view) -->
                  <div class="flex justify-end mt-6 space-x-2">
                    <Dialog v-if="selectedInvoiceForDetail.status === 'DISBURSED' || selectedInvoiceForDetail.status === 'PAID'"> <DialogTrigger as-child>
                      <Button @click="fetchReceiptDetails(selectedInvoiceForDetail.id)" variant="outline" class=" border-gray-700 hover:bg-gray-200 px-6 text-sm rounded-md">
                      ...
                      <DialogContent class="sm:max-w-[625px] shadow-lg rounded-lg">
                        ...
                        <div v-else-if="currentReceipt" class="col-span-1 p-6 md:grid md:grid-cols-2 lg:grid-cols-2 gap-x-4 gap-y-2 text-sm">
                          ...
                          <p class="">Amount:</p><p class=" font-medium">{{ selectedInvoiceForDetail.currency || 'GHS' }} {{ currentReceipt.amount?.toLocaleString() }}</p>
                        ...
                        <DialogFooter class="p-6 border-t">
                          <Button @click="triggerDownloadReceipt(selectedInvoiceForDetail.id)" class=" bg-blue-700 hover:bg-blue-800 px-6 text-sm rounded-md">
                            <Download class="mr-2 h-4 w-4" /> Download Receipt
                          </Button>
                        </DialogFooter>
                      </DialogContent>
                    </Dialog>
                    <Button v-else @click="triggerDownloadReceipt(selectedInvoiceForDetail.id)" class=" bg-blue-700 hover:bg-blue-800 px-6 text-sm rounded-md">
                      <Download class="mr-2 h-4 w-4" /> Download Invoice PDF
                    </Button>
                  </div>
                </div>

                <!-- Bank Details for Invoice Funding (Conditional Card) -->
                <div v-if="selectedInvoiceForDetail.bankFinancingDetails" class="bg-gray-50 p-4 rounded-lg shadow-sm">
                  <h3 class="text-lg font-semibold mb-2">Bank Financing Details</h3>
                  <div class="space-y-1 text-sm">
                    <div class="flex justify-center py-8 items-center">
                      <img :src="selectedInvoiceForDetail.bankFinancingDetails.bankName" :alt="selectedInvoiceForDetail.bankFinancingDetails.bankName"/>
                    </div>
                    <div class="flex justify-between items-center">
                      <p class="font-medium">Bank:</p>
                      <p>{{ selectedInvoiceForDetail.bankFinancingDetails.bankName }}</p>
                    </div>
                    <div class="flex justify-between items-center">
                      <p class="font-medium">Account No.:</p>
                      <p>{{ selectedInvoiceForDetail.bankFinancingDetails.accountNumber }}</p>
                    </div>
                    <div class="flex justify-between items-center">
                      <p class="font-medium">Funded Amount:</p>
                      <p>{{ selectedInvoiceForDetail.currency || 'GHS' }} {{ selectedInvoiceForDetail.bankFinancingDetails.fundedAmount?.toLocaleString() }}</p>
                    </div>
                    <div class="flex justify-between items-center">
                      <p class="font-medium">Purpose:</p>
                      <p class="text-right">{{ selectedInvoiceForDetail.bankFinancingDetails.purpose }}</p>
                    </div>
                  </div>
                </div>
            ...
            <div v-else class="text-center py-10 text-muted-foreground">
              <p class="text-xl font-semibold">Select an Invoice to View Details</p>
              <p class="mt-2">Click on an invoice from the list on the left to see its full details here.</p>
            </div>
          </div>
        </div>
      </div>
...
const selectedKycFile = ref<File | null>(selectedFile)
...
// New state for selected invoice in the main list, initialized to null
...
const fetchInvoices = async () => {
  ...
  try {
    ...
    // Removed the line that automatically selects the first invoice:
