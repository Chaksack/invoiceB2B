...
Date: May 21, 2025
1. Overview
   This document outlines the plan and architecture for an Invoice Financing Application. 
   The platform will allow businesses (users) to upload invoices for financing. 
   Administrators will review these invoices, manage user KYC, and process financing. 
The system will include features for user registration, 2FA, invoice processing (PDF/CSV to JSON), email notifications, fund disbursement tracking, repayment handling, and activity logging.
2. Core Features
   2.1. User Features:
   Registration:
...
4. System Architecture (High-Level)
   +-------------------+      +-------------------+      +---------------------+
   |      User UI      |----->|    GoFiber API    |<---->|     PostgreSQL      |
   ...
   +->|   Redis   |<---------------------+
   +-----------+
   (Cache, Sessions, 2FA OTPs)


Components:
User Interface (Web Application): A frontend application (e.g., built with React, Vue, or server-side Go templates) that users and admins interact with.
GoFiber API Backend:
...
6. Key Workflows & Processes
   ...
   API validates data, creates users record (inactive or KYC pending).
   API publishes 'user_registered' event to RabbitMQ.
   Email Worker consumes event, sends confirmation email.
   User logs in, prompted to complete KYC.
   User submits KYC info/documents.
   API creates kyc_details record (status: 'pending'), stores documents (e.g., S3 or local secure storage, path in DB).
   Admin notified of new KYC submission.
   Admin reviews KYC via admin panel.
   Admin approves/rejects KYC. API updates kyc_details status.
   API publishes 'kyc_status_updated' event to RabbitMQ.
   Email Worker consumes event, sends KYC status email to user.
   If approved, user account is fully activated.
   6.2. User Login with 2FA:
   User submits email/password.
   API validates credentials.
   If valid and 2FA is enabled for the user:
   ...
   Prompt user for OTP.
   User submits OTP.
   API validates OTP against Redis store.
   If valid:
   Generate JWT.
   Set secure, HTTP-only cookie containing JWT.
   Create session entry in Redis (if applicable).
   Grant access.
   If invalid OTP, deny access.
   6.3. Invoice Upload & Processing:
   User (KYC approved) uploads PDF/CSV invoice.
   API receives file, performs basic validation.
   API stores original file securely (e.g., S3 or local volume).
   API creates invoices record (status: 'pending_review', original_file_path set).
   API publishes 'invoice_uploaded' event to RabbitMQ with file path and invoice ID.
   PDF/CSV Worker:
   Consumes message.
   Retrieves file.
   ...
   Admin notified of new invoice for review.
   ...
   Funds Disbursement (Manual/Simulated):
...
9. Email Notifications Summary
   User Registration: Welcome email with confirmation link/info.
   KYC Submitted: Confirmation that KYC info was received.
   KYC Approved: Notification of approval, user can now use full features.
   KYC Rejected/Resubmit: Notification with reason and next steps.
   Invoice Uploaded: Confirmation that invoice was received and is being processed.
   Invoice Approved for Financing: Notification that invoice is approved.
   Invoice Rejected for Financing: Notification with reason.
   Invoice Funds Disbursed: Notification that funds have been sent, with attached receipt.
   Invoice Repayment Due Reminder: (Optional, future feature)
   Invoice Repayment Confirmed: Notification that repayment was successful.
   Password Reset Request: Email with reset link.
   2FA Code (if email OTP is used): Email with OTP.
10. Project Structure (GoFiber - Example)
    /invoice-financing-app
    |-- /cmd
    |   |-- /api                 # Main API application
    |   |   |-- main.go
    |   |-- /pdfworker           # PDF processing worker
    |   |   |-- main.go
    |   |-- /csvworker           # CSV processing worker
    |   |   |-- main.go
    ...
    |   |-- /database            # Database connection, GORM setup
    ...
    |   |-- /models              # GORM models (User, Invoice, etc.)
    |   |-- /repositories        # Data access logic (interacts with GORM)
    |   |-- /services            # Business logic services
    |   |   |-- user_service.go
    |   |   |-- invoice_service.go
    |   |   |-- kyc_service.go
    |   |   |-- notification_service.go (interfaces with RabbitMQ)
    |   |-- /utils               # Utility functions (e.g., password hashing, file processing)
    |   |-- /workers             # Common worker logic, RabbitMQ interaction
    |-- /pkg                     # Shared libraries (if any, less common in typical Go structure)
    |-- /migrations              # Database migration files
    |-- /templates               # HTML templates (if using server-side rendering)
    |-- /uploads                 # Temporary storage for uploads (ensure proper security) - better to use S3
    |-- go.mod
    |-- go.sum
    |-- Dockerfile               # For the API
    |-- Dockerfile.pdfworker
    |-- Dockerfile.csvworker
    |-- Dockerfile.emailworker
    |-- docker-compose.yml
