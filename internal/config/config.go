...
type Config struct {
	...
	RabbitMQURL                      string
	RabbitMQEventExchangeName        string // New: Name of the exchange for events
	RabbitMQUserRegisteredRoutingKey string // New: Routing key for user registered event
...
func LoadConfig(path string) (*Config, error) {
	...
	cfg := &Config{
		...
		RabbitMQEventExchangeName:        getEnv("RABBITMQ_EVENT_EXCHANGE_NAME", "events_exchange"),
		...
		SMTPSenderEmail: getEnv("SMTPSenderEmail", "Invoice App <no-reply@example.com>"), // Corrected key
	...
	if cfg.JWTSecret == "supersecretkey" || cfg.JWTSecret == "your_very_secret_key_for_jwt_change_this" {
